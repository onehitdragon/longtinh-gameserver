 ...
 * </p>
 * 
 * @文件 ResponseStoreGoodsList.java
 * @创建者 DingChu
 * @版本 1.0
 * @时间 2009-11-20 下午02:21:16
 * @描述 ：
 */

public class ResponseStoreGoodsList extends AbsResponseMessage {
     private static Logger log = Logger.getLogger(ResponseStoreGoodsList.class);
	/**
	 * 商店
	 */
	 ...
	 * @param _store
	 */
	public ResponseStoreGoodsList(PersionalStore _store) {
		store = _store;
	}
...
log.info("ResponseStoreGoodsList .. " + store.opened);
		...
		for (int i = 0; i < PersionalStore.MAX_SIZE; i++) {
			...
			if (null != goods) {
				...
				if (goods.goodsType == EGoodsType.EQUIPMENT.value()) {
					...
					yos.writeShort(goods.equipment.getArchetype().getIconID());// 物品图标
					...
					if (goods.equipment.getArchetype() instanceof Weapon) {
						yos.writeByte(1);// 武器
						yos.writeBytes(goods.equipment.getArchetype().getFixPropertyBytes());
						yos.writeByte(goods.equipment.isBind());
						yos.writeByte(goods.equipment.existSeal());
						yos.writeShort(goods.equipment.getCurrentDurabilityPoint());
						yos.writeInt(goods.equipment.getArchetype().getRetrievePrice());
